(function (): void {
    ...
    function PostService(
        ...
        this.getPost = function (): Promise<ApiResponse<FeedPost>> {
            if (!APP_CONFIG.useMockApi) {
                return $http
                    .get<FeedPost>(APP_CONFIG.urlApi + '/api/Comment/GetFeedPost')
                    .catch(function (): Promise<ApiResponse<FeedPost>> {
        ...
        this.addComment = function (payload: AddCommentPayload): Promise<ApiResponse<FeedComment>> {
            if (!APP_CONFIG.useMockApi) {
                return $http
                    .post<FeedComment>(APP_CONFIG.urlApi + '/api/Comment/AddComment', payload)
                    .catch(function (): Promise<ApiResponse<FeedComment>> {
        ...
        function saveMockComment(payload: AddCommentPayload): Promise<ApiResponse<FeedComment>> {
        ...
        function getNextCommentId(): number {
            ...
            return Math.max.apply(null, post.comments.map(function (comment: FeedComment): number {
                return comment.id;
            })) + 1;
        }
